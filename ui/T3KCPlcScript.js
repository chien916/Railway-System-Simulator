...
function plc($CTC_IO, $KC_IN, $KM_IO, $BCN_OUT) {
	...
	$CTC_IO[30] = $KM_IO[0] &&//IF TRACK MODEL IS CONNECTED
			$KM_IO[30];//PASS THE FAILURE MODE OF TRACK MODEL TO CTC, 0 IF CONNECTION LOST
	...
	for (let $_ = 7; $_ >= 0; --$_) $BCN_OUT[2 + $_] =
									(($CTC_IO[0]) ?//IF CTC IS CONNECTED AND UNDER MAINTANANCE MODE
									...
									(!($KM_IO[6]||$KM_IO[7]||$KM_IO[30]));//GIVEN THAT NO FAILURES ARE PRESENT ON TRACK
	//0 IF BOTH CONNECTIONS ARE LOST

	for (let $_ = 7; $_ >= 0; --$_)  $BCN_OUT[10 + $_] =
									 (($KC_IN[0] & $KC_IN[0]) ?//IF WAYSIDE IS CONNECTED AND UNDER MAINTANANCE MODE
																...
																($CTC_IO[0])&&//ELSE IF CTC IS CONNECTED
	...
	//KM::LEFT_SIGNAL
	$KM_IO[16] =
			(($KC_IN[1] && $KC_IN[1]) ?//IF WAYSIDE IS CONNECTED AND UNDER MAINTANANCE MODE
	...
	$KM_IO[18] = //NEW SWITCH POSITION FOR TRACK MODEL
			($KC_IN[1] && $KC_IN[1] && $KC_IN[27]) ?//IF WAYSIDE IS CONNECTED AND UNDER MAINTANANCE MODE
													 $KC_IN[26] ://USE THE MANUAL SWITCH POSITION FROM WAYSIDE
													 ($CTC_IO[0] && $CTC_IO[1]) ?//ELSE IF CTC IS CONNECTED AND UNDER MAINTANANCE MODE
